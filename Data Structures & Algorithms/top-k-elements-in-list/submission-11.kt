class Solution {
    ...
    */
    data class Element(val freq: Int, val value: Int)
    fun topKFrequent(nums: IntArray, k: Int): IntArray {
        // let's practice using heap as well
        val minHeap = PriorityQueue<Element> { a, b ->
            a.freq - b.freq
        }
        val valueToFreq = nums.toList().groupingBy { it }.eachCount()
        valueToFreq.entries.forEach { (key, value) ->
            minHeap.add(Element(freq = value, value = key))
            if (minHeap.size > k) {
                minHeap.poll()
            }
        }
        return minHeap.toList().map { it.value }.toIntArray()
