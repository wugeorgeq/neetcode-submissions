class Solution {
    ...
    */
    fun canFinish(numCourses: Int, prerequisites: Array<IntArray>): Boolean {
        ...
        val satisfiedSet = mutableSetOf<Int>()
        return prerequisites.all {
            satisfies(it[1], prerequisites, list, mutableSetOf<Int>(), satisfiedSet)
        }
    }
    ...
    satisfiedSet: MutableSet<Int>): Boolean {
        ...
        visited.add(current)
        return prereqs.all {
            val sat = satisfies(it, prerequisites, adjList, visited, satisfiedSet)
            if (sat) satisfiedSet.add(it)
            sat
        }
    }
}
